...
public class MoveNN : MonoBehaviour{
	...
	public MoveNN(int n_in, int n_hid, int n_out, int[] hid_l, double ga, double et, double al, int n_c, int l_n)
	{
		gain = ga;
		eta = et;
		alpha = al;
	...
	public void forwardPropagation(double[] x,int hoge)
	{
		a = "";
		double sum;
		//print("NN weight " + x[6]);
		for (int layer = 0; layer < layer_size; layer++)
		{
			...
			else
			{
				...
				a += (Math.Abs(1 - output_value[layer_size - 1][0]));
				a += Convert.ToString(RoundingOutput(output_value[layer_size - 1][1]) - 1);
				//nnai.setInput(a);
				//nnai.inputresult = a;
				if(hoge == 0){
					nnai.ActionDecision(nnai.MoveDecision(a),"enemy");
				}else if(hoge == 1){
					nnai.ActionDecision(nnai.MoveDecision(a),"Player");
				}
