...
public class AttackNN : MonoBehaviour{
	...
	public AttackNN(int n_in, int n_hid, int n_out, int[] hid_l, double ga, double et, double al, int n_c, int l_n)
	{
		gain = ga;
		eta = et;
		alpha = al;
	...
	public void forwardPropagation(double[] x,int hoge)
	{
		...
		for (int layer = 0; layer < layer_size; layer++)
		{
			...
			else
			{
				...
				if(output_value[layer_size - 1][0] < output_value[layer_size - 1][1]){
					a = "01";
				}else if(output_value[layer_size - 1][0] > output_value[layer_size - 1][1]){
					a = "10";
				}else{
					a = "00";
				}
				...
				//nnai.inputresult = a;
				if(hoge == 0){
					nnai.ActionDecision(a,"enemy");
				}else if(hoge == 1){
					nnai.ActionDecision(a,"Player");
				}
